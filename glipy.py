...
class Param:
	def __init__(self, name, shortname, type, doc):
		self.name 		= name
	...
	def __repr__(self):
		return repr((self.name, self.shortname, self.type, self.doc[0:10]))
...
def parse_commandline():
	...
	def get_typed_value(k, value=None, short=True):
		...
		t = [x for x in syntax if x.shortname == k] if short else [x for x in syntax if x.name == k]

		if t == []:
			raise Exception("Unknown key %s" % key)

		if t[0].type == Type.FLAG:
			return True
		elif value is None:
			if short:
...
def get_decorator(type, value, *args, **kwargs):
	def decorator(callable):
		name = kebab(callable.__name__) if "name" not in kwargs else kwargs["name"]
		short = kwargs["shortname"] if "shortname" in kwargs else name[0]
		if short in [x.shortname for x in syntax]:
			raise Exception("Duplicating shortname for command-line argument --%s: '%s', is already in use by --%s" % (name, short, [x.name for x in syntax if x.shortname == short][0]))
		syntax.append(Param(name, short, type, callable.__doc__))
...
def init():
	...
	if help() or len(sys.argv) == 1:
		...
		shorts = ''.join([x.shortname for x in syntax])
		longs = ' --'.join([x.name for x in syntax])
		print("Usage: -%s --%s" % (shorts, longs))
		for arg in syntax:
			print("\t--%s\t-%s\t%s" % (arg.name, arg.shortname, arg.doc))
		exit()
